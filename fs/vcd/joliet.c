 ...
 */
static int uni16_to_x8(unsigned char *ascii, u16 *uni, int len, struct nls_table *nls)
{	wchar_t *ip, ch;
	unsigned char *op;
	ip = uni;
	op = ascii;
...
static int wcsntombs_be(__u8 *s, const __u8 *pwcs, int inlen, int maxlen)
{
	const __u8 *ip;
	__u8 *op;
	int size;
	__u16 c;
	//printk("-------------------------joliet.c:wcsntombs_be()_ST_AnyoneUseThis???\n");
	op = s;
	ip = pwcs;
