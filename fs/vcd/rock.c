...
{ //mips_iso calls this
 ...
 repeat:
  ...
  if (buffer) kfree(buffer);	//The buffer is defined in macro MAYBE_CONTINUE
...
static char *get_symlink_chunk(char *rpnt, struct rock_ridge *rr, char *plimit)
{
	...
	struct SL_component *oldslp;
	struct SL_component *slp;
	printk("-------------------------rock.c:get_symlink_chunk()_ST_AnyoneUseThis???\n");
	slen = rr->len - 5;
	slp = &rr->u.SL.link;
	while (slen > 1) {
		rootflag = 0;
...
{  //mips_iso calls this
   ...
   result=parse_rock_ridge_inode_internal(de,inode,0);
...
static int rock_ridge_symlink_readpage(struct file *file, struct page *page)
{
	...
	unsigned char *chr;
	struct rock_ridge *rr;
	printk("-------------------------rock.c:rock_ridge_symlink_readpage()_ST_AnyoneUseThis???\n");
	...
	lock_kernel();
	bh = sb_bread(inode->i_sb, block);	//R01
	if (!bh)
		goto out_noread;
	...
	MAYBE_CONTINUE(repeat, inode);
	if (buffer)	//The buffer is defined in macro MAYBE_CONTINUE
		kfree(buffer);
