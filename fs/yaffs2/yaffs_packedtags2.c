...
debug_nand("-------[%s]------- \n", __FUNCTION__);
	T(YAFFS_TRACE_MTD,
	  (TSTR("packed tags obj=%d chunk=%d byte=%d seq=%d\n" TENDSTR),
...
void  yaffs_PackTags2TagsPart(yaffs_PackedTags2TagsPart * ptt, const yaffs_ExtendedTags * t)
{
	...
	/*
	  printk("[%s](ptt)packed tags obj %d chunk %d byte %d seq %d\n",
	  __FUNCTION__, ptt->objectId, ptt->chunkId, ptt->byteCount,
	  ptt->sequenceNumber);

	  printk("[%s](t)packed tags obj %d chunk %d byte %d seq %d\n",
	  __FUNCTION__, t->objectId, t->chunkId, t->byteCount,
	  t->sequenceNumber);
	*/
