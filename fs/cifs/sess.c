...
//js	strcpy(bcc_ptr, init_utsname()->release);
//js	bcc_ptr += strlen(init_utsname()->release) + 1;
...
/* We look for obvious messed up bcc or strings in response so we do not go off
   ...
   terminating last Unicode string in response  */
	...
	cFYI(1,("words left4: %d",words_left));
		...
		const struct nls_table *nls_cp)
{
	...
	str_area = kzalloc(2000, GFP_KERNEL);
	bcc_ptr = str_area;
...
/* #ifdef CONFIG_CIFS_DEBUG2
		...
		char * v2_sess_key = 
			kzalloc(sizeof(struct ntlmv2_resp), GFP_KERNEL);
