...
cFYI(1, ("js1 - kernel_recvmsg"));
		length =
		    kernel_recvmsg(csocket, &smb_msg,
				 &iov, 1, 4, 0 /* BB see socket.h flags */);
cFYI(1, ("js2 - kernel_recvmsg"));
...
//js			if (!try_to_freeze() && (length == -EINTR)) {
			if (!try_to_freeze(PF_FREEZE) && (length == -EINTR)) { //js
...
//js		char *nodename = utsname()->nodename;
		char *nodename = system_utsname.nodename;  //js
	...
	while ((data = strsep(&options, separator)) != NULL) {
		...
		if (strnicmp(data, "user_xattr",10) == 0) {/*parse before user*/
			...
			if ((temp_len = strnlen(value, 300)) < 300) {
				vol->UNC = kzalloc(temp_len+1,GFP_KERNEL);
				if(vol->UNC == NULL)
					return 1;
			...
			if (strnlen(value, 256) < 256) {
				...
				if(value[0] != '/')
					temp_len++;  /* missing leading slash */
                                vol->prepath = kzalloc(temp_len+1,GFP_KERNEL);
                                if(vol->prepath == NULL)
                                        return 1;
				if(value[0] != '/') {
					vol->prepath[0] = '/';
			...
			unsigned char ** preferrals, int remap)
{
	...
	*pnum_referrals = 0;
	...
	if (pSesInfo->ipc_tid == 0) {
		temp_unc = kzalloc(2 /* for slashes */ +
...
//printk(KERN_WARNING " " __FILE__ ": %d jason UNC = %s \n",__LINE__,volume_info.UNC);
...
//printk(KERN_WARNING " " __FILE__ ": %d jason tcon UNC = %s \n",__LINE__,tcon->treeName);            
		} else {
			tcon = tconInfoAlloc();
			if (tcon == NULL)
				rc = -ENOMEM;
				...
				if ((strchr(volume_info.UNC + 3, '\\') == NULL)
                    ...
                    tcon->nls=cifs_sb->local_nls;  //jason add
//printk(KERN_WARNING " " __FILE__ ": %d jason tcon UNC = %s \n",__LINE__,tcon->treeName);            
				}
				if (!rc) {
	      ...
	      const struct nls_table *nls_codepage)
{
	...
	if (ses->capabilities & CAP_UNICODE) {
		...
		bcc_ptr += 2 * bytes_returned;
		bytes_returned =
//js		    cifs_strtoUCS((__le16 *) bcc_ptr, utsname()->release,
...
//js		strcpy(bcc_ptr, utsname()->release);
//js		bcc_ptr += strlen(utsname()->release) + 1;
		strcpy(bcc_ptr, system_utsname.release);  //js
		bcc_ptr += strlen(system_utsname.release) + 1; //js
...
/*	if(ntlmv2_support)
	...
	if (ses->capabilities & CAP_UNICODE) {
		...
		bcc_ptr += 2 * bytes_returned;
		bytes_returned =
//js		    cifs_strtoUCS((__le16 *) bcc_ptr, utsname()->release, 32,
...
//printk(KERN_WARNING " " __FILE__ ": %d jason tree = %s codepage=%s\n",__LINE__,tree,nls_codepage->charset);
	} else {		/* ASCII */
		strcpy(bcc_ptr, tree);
...
//js	if (ses)
//js		schedule_timeout_interruptible(msecs_to_jiffies(500));
			...
			if (!rc) {
				if(ntlmv2_flag) {
					...
					if(CalcNTLMv2_partial_mac_key(pSesInfo, 
						...
						v2_response = kzalloc(16 + 64 /* blob */, GFP_KERNEL);
					if(v2_response) {
