...
/* BB Add logic to parse referrals here */
		...
		if (rc || (pSMBr->ByteCount < 17))      /* BB also check enough total bytes returned */
			...
			*targetUNCs = 
				kzalloc(name_len+1+ (*number_of_UNC_in_array),GFP_KERNEL);
			if(*targetUNCs == NULL) {
				rc = -ENOMEM;
				goto GetDFSRefExit;
			}
...
/* BB VERIFY verify which is correct for above BB */
	...
	if (rc) {
		...
		dnotify_req = (struct dir_notify_req *) kzalloc(
