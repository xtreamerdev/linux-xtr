...
//
static int __init rtd2885_otg_init(void)
{
    ...
    if (!lmdev) {
    	printk("%s [Error] : Can't allocate memory for lmdev.\n",__FUNCTION__);
    	return;
    }
           
    memset(lmdev, 0, sizeof(struct lm_device));

    lmdev->resource.start = RTD_OTG_BASE;
    lmdev->resource.end = lmdev->resource.start + 0x0fffffff;
    lmdev->resource.flags = IORESOURCE_MEM;
    lmdev->irq = DWC_OTG_IRQ_NUM;  
    lmdev->id = 1;
    
    // rtd_otg_enable_irq(lmdev->irq);
    
    return lm_device_register(lmdev);  
}

device_initcall(rtd2885_otg_init);
