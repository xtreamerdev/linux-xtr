			     ...
			     union iwreq_data *wrqu, char *b)
{
	...
	if(ieee->iw_mode == IW_MODE_INFRA){ 
		//ret = -EOPNOTSUPP;
		ret = 0;//wpa will check it, in some UI wpa will fail
		goto out;
	}
...
//the wrqu->name size is IFNAMSIZ=16, so the name must
//not bigger than IFNAMSIZ, or iwconfig will panic in
//some kernel version link:RedHat 2.4.20-8smp.
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	if (ieee->mode & (IEEE_N_24G | IEEE_N_5G))
		strcat(wrqu->name, "n");
// to avoid stack protector issue, mark it temperly.
#if 0	
	if((ieee->state == RTLLIB_LINKED) || 
		(ieee->state == RTLLIB_LINKED_SCANNING))
