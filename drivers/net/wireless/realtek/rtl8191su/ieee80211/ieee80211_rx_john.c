			   ...
			   struct ieee80211_crypt_data *crypt)
{
	...
	if (crypt == NULL || crypt->ops->decrypt_mpdu == NULL)
		return 0;
#if 1
	if (ieee->hwsec_support && ieee->hwsec_active)
	{
...
//	u16 qos_ctl = 0;
	...
	if(1||!ieee->current_network.qos_data.active|| !IsDataFrame(skb->data) || IsLegacyDataFrame(skb->data)){
...
//added by amy for reorder
	if(ieee->pHTInfo->bCurRxReorderEnable == false || 1||pTS == NULL){
//added by amy for reorder
		for(i = 0; i<rxb->nr_subframes; i++) {
	...
	struct ieee80211_rx_stats *stats)
{
	...
	if(is_same_network(&ieee->current_network, &network)) {
		...
		if(ieee->current_network.berp_info & ERP_UseProtection)
	...
	if (is_beacon(beacon->header.frame_ctl)&&is_same_network(&ieee->current_network, &network)&&\
