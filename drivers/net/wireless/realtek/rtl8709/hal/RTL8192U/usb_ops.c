...
u8 usb_read8(struct intf_hdl *intf_hdl, u32 addr)
{
	...
	// It just for developing purpose, we shall make all MAC register looks like 0x0NNN on 8192U
	// 1111 1111 xxxx xxxxb
	if((addr&0xff00)==0xff00){
		status = usb_control_msg(	udev,
								...
								HZ/2);
	// USB SIE register.
	// Set wValue to the 0xfeZZ and wregisterIndex to 0x0000 if we want to access SIE.0xZZ.
	// 1111 1110 xxxx xxxxb
		...
		status = usb_control_msg(	udev,
								...
								1,
								HZ/2);
	// >=1xxx xxxx xxxx xxxxb
	}else if(addr>=0x8000){
		DEBUG_INFO(("\n Invalid register address (%x) read access!!\n",addr));
	// MAC register page 0-3.
	// Set wValue to the 0xffYY and wregisterIndex to 0x000X if we want to access MAC.0xXYY, i.e. page 0xX, offset 0xYY.
	// < 0xxx xxxx xxxx xxxxb
	}else if(addr<0x8000){
		status = usb_control_msg(	udev,
								usb_rcvctrlpipe(udev,0),
								RTL8187_REQ_GET_REGS,
		...
		DEBUG_INFO(("\n Invalid register address read access!!\n"));
	}
...
void usb_write8(struct intf_hdl *pintfhdl, u32 addr, u8 val)
{
	...
	// 1111 1111 xxxx xxxxb
	if((addr&0xff00)==0xff00){
//		DEBUG_ERR(("\n ff register write access addr=%x val=%x\n", addr, val));
		status = usb_control_msg(	udev,
...
//		DEBUG_ERR(("\n fe register write access addr=%x val=%x\n", addr, val));
		status = usb_control_msg(	udev,
								usb_sndctrlpipe(udev,0),
								RTL8187_REQ_SET_REGS,
								USB_ENDPOINT_OUT|USB_TYPE_VENDOR,//USB_ENDPOINT_OUT|USB_TYPE_VENDOR,
								addr,
								0,
								&val,
								1,
...
//		DEBUG_ERR(("\n Normal register write access addr=%x val=%x\n", addr, val));
		status = usb_control_msg(	udev,
								usb_sndctrlpipe(udev,0),
								RTL8187_REQ_SET_REGS,
								USB_ENDPOINT_OUT|USB_TYPE_VENDOR,
