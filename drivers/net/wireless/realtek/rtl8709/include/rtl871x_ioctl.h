...
extern struct iw_handler_def  r8711_handlers_def;
...
#define DEBUG_OID(dbg, str)     		\
       if((!dbg))				    			\
      	{					    			\
	   DEBUG_INFO(("%s(%d): %s", __FUNCTION__, __LINE__, str));	\
      	}			
...
struct oid_par_priv
{
	_nic_hdl		adapter_context;
	NDIS_OID		oid;
	PVOID			information_buf;
