...
_func_enter_;	
	...
	pframe=get_xmit_payload_start_addr((struct xmit_frame*)pxmitframe);
	...
	if((pattrib->encrypt==_WEP40_)||(pattrib->encrypt==_WEP104_)){
		...
		for(curfragnum=0;curfragnum<pattrib->nr_frags;curfragnum++){
			...
			if((curfragnum+1)==pattrib->nr_frags){	//4 the last fragment
				...
				*((unsigned long *)crc)=getcrc32(payload,length);
...
_func_enter_;	
	...
	if((prxattrib->encrypt==_WEP40_)||(prxattrib->encrypt==_WEP104_)){
		...
		//4 calculate icv and compare the icv
		*((unsigned long *)crc)=getcrc32(payload,length-4);
		if(crc[3]!=payload[length-1] || crc[2]!=payload[length-2] || crc[1]!=payload[length-3] || crc[0]!=payload[length-4])
			DEBUG_ERR(("wep_decrypt:icv error crc[3](%x)!=payload[length-1](%x) || crc[2](%x)!=payload[length-2](%x) || crc[1](%x)!=payload[length-3](%x) || crc[0](%x)!=payload[length-4](%x)\n",
...
_func_enter_;
	...
	if(pattrib->encrypt==_TKIP_){
		...
		if (stainfo!=NULL){
			DEBUG_ERR(("tkip_encrypt: stainfo!=NULL!!!\n"));
			...
			for(curfragnum=0;curfragnum<pattrib->nr_frags;curfragnum++){
				...
				if((curfragnum+1)==pattrib->nr_frags){	//4 the last fragment
					length=pattrib->last_txcmdsz-pattrib->hdrlen-pattrib->iv_len- pattrib->icv_len;
					DEBUG_INFO(("pattrib->iv_len =%x, pattrib->icv_len =%x\n", pattrib->iv_len,pattrib->icv_len));
					*((u32 *)crc)=getcrc32(payload,length);/* modified by Amy*/
...
_func_enter_;
	...
	if(prxattrib->encrypt==_TKIP_){
		...
		if (stainfo!=NULL){
			DEBUG_ERR(("tkip_decrypt: stainfo!=NULL!!!\n"));
			prwskey=&stainfo->dot118021x_UncstKey.skey[0];
			prwskeylen=16;
...
_func_enter_;		
	...
	if((pattrib->encrypt==_AES_)){
		...
		stainfo=get_stainfo(&padapter->stapriv ,&pattrib->ra[0] );
		if (stainfo!=NULL){
			DEBUG_ERR(("aes_encrypt: stainfo!=NULL!!!\n"));
			prwskey=&stainfo->dot118021x_UncstKey.skey[0];
			prwskeylen=16;
...
_func_enter_;			
	...
	for(i=0;i<8;i++){
		if(pframe[hdrlen+8+plen-8+i] != message[hdrlen+8+plen-8+i])
			DEBUG_ERR(("aes_decipher:mic check error mic[%d]: pframe(%x) != message(%x) \n",
...
_func_enter_;	
	...
	if((prxattrib->encrypt==_AES_)){
		...
		stainfo=get_stainfo(&padapter->stapriv ,&prxattrib->ta[0] );
		if (stainfo!=NULL){
			DEBUG_ERR(("aes_decrypt: stainfo!=NULL!!!\n"));
			prwskey=&stainfo->dot118021x_UncstKey.skey[0];
			prwskeylen=16;
