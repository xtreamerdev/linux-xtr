...
void init_phyinfo(_adapter  *adapter, struct setphyinfo_parm* psetphyinfopara)
{
	...
	switch(country_string)
	{
		...
		case EUROPE:
			...
			set_channelset_bg(adapter, &psetphyinfopara->class_sets[0], 13, 14);
		...
		default:
			...
			DEBUG_ERR(("Country string in EEPROM has not been defined."));
			break;
...
u8 writephyinfo_fw(_adapter *padapter, u32 addr)
{
	...
	memset((unsigned char *)psetphyinfopara, 0, sizeof (struct setphyinfo_parm));
