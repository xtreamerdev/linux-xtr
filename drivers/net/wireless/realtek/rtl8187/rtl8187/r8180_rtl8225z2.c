...
//-
static u8 ZEBRA2_CCK_TXPOWER_C14[]={
...
void rtl8225z2_SetTXPowerLevel(struct net_device *dev, short ch)
{
	...
	u32 value;
	u8 *cck_power_table;
	s8 max_cck_power_level = 15;
	s8 min_cck_power_level = 0;
	s8 max_ofdm_power_level = 15;
	s8 min_ofdm_power_level = 0;	
	s8 cck_power_level = (s8)priv->chtxpwr[ch];
	s8 ofdm_power_level = (s8)priv->chtxpwr_ofdm[ch];
	...
	if(ch == 14)
	{
		cck_power_table = ZEBRA2_CCK_TXPOWER_C14;
	}
	else
	{
		cck_power_table = ZEBRA2_CCK_TXPOWER;
	}
	...
	for(i=0;i<8;i++){
		...
		write_nic_dword(dev,PhyAddr, ((0x010000c4+i) | value ));
	}
	...
	if(ofdm_power_level <= 11){
		write_nic_dword(dev, PhyAddr,0x00005C87);
		write_nic_dword(dev, PhyAddr,0x00005C89);
	}
	else if(ofdm_power_level <= 17){
		write_nic_dword(dev, PhyAddr,0x00005487);
		write_nic_dword(dev, PhyAddr,0x00005489);
	}
	else {
		write_nic_dword(dev, PhyAddr,0x00005087);
		write_nic_dword(dev, PhyAddr,0x00005089);
	}
}
#if 0
