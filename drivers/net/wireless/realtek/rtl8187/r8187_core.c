...
//		RCR_ACF |			//accept control frame for SW AP needs PS-poll, 2005.07.07, by rcnjko.
	...
	{
		priv->bLowNoiseAmplifier = true;
		//RT_TRACE(COMP_INIT, DBG_LOUD, ("Support LNA\n"));
	}
	else
	{
		priv->bLowNoiseAmplifier = false;
...
static void MacConfig_87BASIC(struct net_device *dev)
{
	...
	if ( (!priv->usbhighspeed) || (priv->card_8187_Bversion > VERSION_8187B_B))
...
void ZEBRA_Config_87BASIC_HardCode(struct net_device* dev)
{
	...
	for(i=1;i<=95;i++)
	{
		...
		if( !priv->bLowNoiseAmplifier){//Non LNA settings.
			write_rtl8225(dev, 0x02, ZEBRA_RF_RX_GAIN_TABLE[i]); 
			mdelay(1);
		}
		else {//LNA settings.
			write_rtl8225(dev, 0x02, ZEBRA_RF_RX_GAIN_LNA_TABLE[i]); 
			mdelay(1);
		}
...
#endif		
#if 0		
		write_phy_ofdm(dev,data&0xff,(data>>8)&0x7f);
		write_phy_ofdm(dev,addr&0xff,(addr>>8)&0x7f);
#endif
#if 0
...
//	struct ieee80211_device *ieee = priv->ieee80211;
	...
	//bool		bUNIVERSAL_CONTROL_AGC = false;
	bool		bUNIVERSAL_CONTROL_AGC = TRUE;
	//bool		bUNIVERSAL_CONTROL_ANT = false;
	bool		bUNIVERSAL_CONTROL_ANT = TRUE;
...
GetHardwareVersion8187( struct net_device	*dev )
{
	...
	case 0x00:
		priv->card_8187_Bversion= VERSION_8187B_B;
		printk(KERN_INFO "Hardware version is 8187B_B\n");
		break;

	case 0x01:
		priv->card_8187_Bversion = VERSION_8187B_D;
		printk(KERN_INFO "Hardware version is 8187B_D\n");
		break;

	case 0x02:
		priv->card_8187_Bversion = VERSION_8187B_E;
		printk(KERN_INFO "Hardware version is 8187B_E\n");
		break;
	...
	if(NIC_8187 == priv->card_8187) {
		...
		*/	
		...
		if(read_nic_byte_E(dev, 0xFE11) & 0x01)
			priv->usbhighspeed = true;
		else
			priv->usbhighspeed = false;
...
#ifdef THOMAS_RATEADAPTIVE
				priv->stats.RecvSignalPower = RX_PWDB;
#endif
...
//
bool IncludedInSupportedRates(struct net_device *dev, u8 TxRate)
{
	struct r8180_priv *priv = (struct r8180_priv *)ieee80211_priv(dev);
	struct ieee80211_device *ieee = priv->ieee80211;
