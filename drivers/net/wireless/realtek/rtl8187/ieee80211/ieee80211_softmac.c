...
static void ieee80211_sta_wakeup(struct ieee80211_device *ieee, short nl);
...
static inline void softmac_ps_mgmt_xmit(struct sk_buff *skb, struct ieee80211_device *ieee)
{
...
static struct sk_buff *ieee80211_get_beacon_(struct ieee80211_device *ieee);
...
static void ieee80211_softmac_scan_wq(struct ieee80211_device *ieee)
{	
...
static struct sk_buff* ieee80211_assoc_resp(struct ieee80211_device *ieee, u8 *dest)
{
	struct sk_buff *skb;
	u8* tag;
...
static struct sk_buff* ieee80211_auth_resp(struct ieee80211_device *ieee,int status, u8 *dest)
{
	struct sk_buff *skb;
	struct ieee80211_authentication *auth;
...
static inline struct sk_buff *ieee80211_association_req(struct ieee80211_network *beacon,struct ieee80211_device *ieee)
{
...
#endif	
	u8 wpa_cipher_suite_count = beacon->wpa_ie[12];
	unsigned int rate_len = ieee80211_MFIE_rate_len(ieee);
	...
	int len = 0; 
	unsigned int wpa_len = 0;
	if(beacon->wpa_ie_len)
	wpa_len = beacon->wpa_ie_len - (wpa_cipher_suite_count-1)*4;  //get the correct wpa_len for association
...
//FIXME			
			...
			assoc_wpa_buff = kmalloc(64, GFP_ATOMIC);
			memset(assoc_wpa_buff, 0, 64);
			if (wpa_cipher_suite_count == 1)
				memcpy(assoc_wpa_buff, beacon->wpa_ie, wpa_len);
			else 
			{
				memcpy(assoc_wpa_buff, beacon->wpa_ie, 18);
				assoc_wpa_buff[1] = wpa_len-2;
				assoc_wpa_buff[12] = 1;
				assoc_wpa_buff[13] = 0;
				assoc_wpa_buff[17] = ieee->pairwise_key_type;
				memcpy(assoc_wpa_buff+18, beacon->wpa_ie+18+(wpa_cipher_suite_count-1)*4, wpa_len-18);
			...
			for(i=0;i<wpa_len;i++){
				p += sprintf(p, "%02x", assoc_wpa_buff[i]);
			}
			p += sprintf(p, ")");
			...
			memcpy(tag, assoc_wpa_buff, wpa_len);
...
static void ieee80211_auth_challenge(struct ieee80211_device *ieee, u8 *challenge, int chlen)
{
	u8 *c;	
	struct sk_buff *skb;
	struct ieee80211_network *beacon = &ieee->current_network;
...
static void ieee80211_associate_complete_wq(struct ieee80211_device *ieee)
{
...
#ifdef THOMAS_RATEADAPTIVE
{
	...
	for( i=0 ; i<ieee->current_network.rates_len ; i++){
		if( i <4 )
			ieee->SupportedRates[i] = ieee->current_network.rates[i] - 0x80;
		else
			ieee->SupportedRates[i] = ieee->current_network.rates[i];
	}
	for( j=0 ; j<ieee->current_network.rates_ex_len ; j++)
		ieee->SupportedRates[i+j] = ieee->current_network.rates_ex[j];
	ieee->SupportedRates_length = i + j;
}
#endif

	netif_carrier_on(ieee->dev);

	// add by cfyeh : 2009/03/25 +++
...
static void ieee80211_associate_procedure_wq(struct ieee80211_device *ieee)
{
	ieee->sync_scan_hurryup = 1;
	down(&ieee->wx_sem);
...
inline void ieee80211_softmac_new_net(struct ieee80211_device *ieee, struct ieee80211_network *net)
{
	...
	if (ieee->iw_mode == IW_MODE_INFRA || ieee->iw_mode == IW_MODE_ADHOC){
		...
		ssidbroad =  (!(net->ssid_len == 0 || net->ssid[0]== '\0'))&&(net->ssid_len == ieee->current_network.ssid_len);
...
static void ieee80211_sta_ps_send_null_frame(struct ieee80211_device *ieee, short pwr)
{
...
static short ieee80211_sta_ps_sleep(struct ieee80211_device *ieee, u32 *time_h, u32 *time_l)
{	
			...
			u16 stype)
{
	...
	switch (WLAN_FC_GET_STYPE(header->frame_ctl)) {
		...
		case IEEE80211_STYPE_REASSOC_RESP:
			...
			//printk(KERN_WARNING "*******************received [RE]ASSOCIATION RESPONSE\n");
		...
		case IEEE80211_STYPE_DEAUTH:
			...
			*/
			if ((ieee->softmac_features & IEEE_SOFTMAC_ASSOCIATE) &&
				ieee->state == IEEE80211_LINKED && 
				ieee->iw_mode == IW_MODE_INFRA){
				//2007-11-29 add site survey here to find new ap information and delay associate work 4400ms , added by thomas
				...
				notify_wx_assoc_event(ieee);
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,5,0)					
				queue_delayed_work(ieee->wq, &ieee->associate_procedure_wq,4400);
#else
				schedule_task(&ieee->associate_procedure_wq);
#endif
			}
...
static void ieee80211_start_ibss_wq(struct ieee80211_device *ieee)
{
...
static void ieee80211_associate_retry_wq(struct ieee80211_device *ieee)
{
	unsigned long flags;
...
static void ieee80211_wpa_assoc_frame(struct ieee80211_device *ieee, char *wpa_ie, int wpa_ie_len)
{
