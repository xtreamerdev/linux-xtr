...
//	if (network->ssid_len == 0) {  //YJ,modified,090701
                               ...
                               struct iw_point *encoding, struct iw_encode_ext *ext, int entry)
{
	...
	printk("idx in set enc %d \n",idx);
		...
		{
			//crypt = &ieee->cryptlist[idx+1]->crypt[0];
...
int rtllib_wx_set_gen_ie(struct rtllib_device *ieee, u8 *ie, size_t len)
{
	...
	if (len) {
		...
		if ((eid == MFIE_TYPE_GENERIC) && (!memcmp(&ie[2], wps_oui, 4))) {
			...
			//printk("rtllib_wx_set_gen_ie(): set WPS_IE, len=%d\n", len);
	...
	if (ieee->wps_ie)
		kfree(ieee->wps_ie);
	ieee->wps_ie = NULL;		
	//set wpa_ie if any and if wps_ie doesn't exist
	if (len) {
		if (len != ie[1]+2) {
			//printk("len:%d, ie:%d\n", len, ie[1]);
			return -EINVAL;
