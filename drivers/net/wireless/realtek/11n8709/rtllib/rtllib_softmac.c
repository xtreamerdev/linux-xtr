...
MgntQuery_TxRateExcludeCCKRates(struct rtllib_device *ieee)
{
	...
	for( i = 0; i < ieee->current_network.rates_len; i++)
	{
		...
		//printk("MgntQuery_TxRateExcludeCCKRates(), BasicRate = 0x%x\n", BasicRate);
		if(!rtllib_is_cck_rate(BasicRate))
		{
			if(QueryRate == 0)
			{
				QueryRate = BasicRate;
				//printk("Find first BasicRate = 0x%x\n", QueryRate);
			}
			else
			{
				...
				{
					QueryRate = BasicRate;
					//printk("Find small BasicRate = 0x%x\n", QueryRate);
				}
			}
		}		
	}
...
u8 MgntQuery_MgntFrameTxRate(struct rtllib_device *ieee)
{
	...
	if(pHTInfo->IOTAction & HT_IOT_ACT_WA_IOT_Broadcom)
	{
		...
		//printk("MgntQuery_MgntFrameTxRate(), rate = 0x%x\n", rate);
	}
	else
		rate = ieee->basic_rate & 0x7f;
#endif
...
inline struct sk_buff *rtllib_probe_req(struct rtllib_device *ieee)
{
	...
	if(!extMore)
		...
		skb = dev_alloc_skb(sizeof(struct rtllib_probe_request) +
			    2 + len + rate_len+128+ieee->tx_headroom); // MESHID + CAP
#else
...
void ext_rtllib_send_beacon_wq(void *data)
{
	...
	//struct rtllib_device *ieee = container_of(((struct work_struct *)data), (struct rtllib_device), (ext_send_beacon_wq));
	struct sk_buff *skb;
...
//		(MSECS( ieee->current_network.beacon_interval -5));
	...
	//SPIN_LOCK_IEEE_BCN(&ieee->beacon_lock);
//	if(ieee->beacon_txing)
//		add_timer(&ieee->beacon_timer);
	//SPIN_UNLOCK_IEEE_BCN(&ieee->beacon_lock);
}
#endif
...
void rtllib_ibss_wait_timeout(unsigned long _ieee)
{
	...
	//del_timer_sync(&ieee->ibss_wait_timer);//del timer here will crash
...
out:
	...
	//after scan wether linked or nolink we ask wpa to get scan.
	{
	//printk("###################>%s()ask UI to get scan\n", __func__);
	//iwreq_data wrqu;
...
void rtllib_softmac_scan_wq(void *data)
{
	...
	//If synchronizing scan is already in progress, do nothing. YJ,add,090603
#ifdef _RTL8192_EXT_PATCH_
...
void rtllib_start_scan(struct rtllib_device *ieee)
{
	...
	ieee->rtllib_ips_leave_wq(ieee->dev);//added by amy for IPS 090331
#endif
...
//	printk("===============>tmp_ht_cap_len is %d,tmp_ht_info_len is %d, tmp_generic_ie_len is %d\n",tmp_ht_cap_len,tmp_ht_info_len,tmp_generic_ie_len);	
	...
	if(ieee->qos_support){
		...
		{
			//disable QoS in Ad-hoc mode by thomas 2009-10-14.
			//wmm_len = 1;
...
struct sk_buff* rtllib_ext_probe_resp_by_net(struct rtllib_device *ieee, u8 *dest, struct rtllib_network *net)
{
	...
	int wpa_ie_len = 0;//ieee->wpa_ie_len;;
	u8 erpinfo_content = 0;
#if 0
	u8* tmp_ht_cap_buf=NULL;
	u8 tmp_ht_cap_len=0;
	...
	if(ieee->current_mesh_network.capability & WLAN_CAPABILITY_IBSS) // use current_mesh_network here
		atim_len = 4;
	else
		atim_len = 0;
	...
	if ((IW_MODE_MESH==ieee->iw_mode))//modified by amy 081015
	{
...
//b	
	...
	beacon_buf->beacon_interval = 
		cpu_to_le16(ieee->current_mesh_network.beacon_interval);  // use current_network here
	beacon_buf->capability = 
...
//pbuf = set_ie(pbuf, _SUPPORTEDRATES_IE_, bssrate_len, pbssrate, &frlen);
	...
	*(tag++) = MFIE_TYPE_DS_SET;
	*(tag++) = 1;
	*(tag++) = ieee->current_mesh_network.channel;  // use current_network here
...
inline struct sk_buff *rtllib_association_req(struct rtllib_network *beacon,struct rtllib_device *ieee)
{
	...
	u8 realtek_ie_len=0;
	int wpa_ie_len= ieee->wpa_ie_len;
	int wps_ie_len = ieee->wps_ie_len;//for wps
	...
	//We must use correct date rate in probe response or beacon of current connecting AP.
	...
	//get assocreq_ies for user space
...
void rtllib_auth_challenge(struct rtllib_device *ieee, u8 *challenge, int chlen)
{
	...
	struct sk_buff *skb;
	struct rtllib_network *beacon = &ieee->current_network;
//	int hlen = sizeof(struct rtllib_authentication);
...
//	if((ieee->iw_mode == IW_MODE_MESH) &&(ieee->only_mesh == 0))//AMY del 090218
	{
		if(ieee->current_mesh_network.channel != ieee->current_network.channel)
		{
			...
			//Inform peers to perform link close
...
//send assocreq_ies & assocresp_ies to user space, 
//should before notify_wx_assoc_event for linked
static void rtllib_sta_send_associnfo(struct rtllib_device *ieee)
{
	...
	//if (!ieee->assocreq_ies && !ieee->assocresp_ies)
		return;

	//printk("#################%s(), assocreq_ies_len:%d,assocresp_ies_len:%d\n", 
	//	__func__, ieee->assocreq_ies_len, ieee->assocresp_ies_len);
...
void rtllib_associate_procedure_wq(void *data)
{
	...
	if(ieee->rtllib_ips_leave != NULL)
        	ieee->rtllib_ips_leave(ieee->dev);//added by amy for IPS 090331
#endif
	down(&ieee->wx_sem);
...
#ifdef _RTL8192_EXT_PATCH_
// based on rtllib_associate_procedure_wq
...
#ifdef ENABLE_AMSDU   
						//Back to default setting since some current ht info may be changed in adhoc mode.
...
//added by amy for adhoc 090402
					...
					//we set adhoc wireless mode based on peer mode, if we support N mode.
					ieee->SetWirelessMode(ieee->dev, ieee->current_network.mode);
					mod_timer(&ieee->ibss_wait_timer,jiffies+(MSECS(20000)));//wait beacon 20s.FIXME 090403														
#else
					if(rtllib_is_54g(&ieee->current_network) && 
						(ieee->modulation & RTLLIB_OFDM_MODULATION)){
						ieee->rate = 108;
...
static short probe_rq_parse(struct rtllib_device *ieee, struct sk_buff *skb, u8 *src)
{
	...
	//RTLLIBDMESG("Card MAC address is "MACSTR, MAC2STR(src));
...
//1) rtllib_sta_ps_send_null_fram(epwr=1) called only the first time when enter Leisure PS and rcv beacon to sleep. 
//   or sta_sleep change from 0->2
//2) we don't call rtllib_sta_ps_send_null_fram(epwr=1) every beacon sleep (sta_sleep = 1)
//3) after we have tell AP we sleep use rtllib_sta_ps_send_null_frame(pwr=1), we will not call it(pwr=0) when wake up.
//4) we just call rtllib_sta_ps_send_pspoll_frame to polling pkt buffered in AP to avoid AP send our pkt when we sleep.
//5) just call rtllib_sta_ps_send_null_frame(pwr=0) when we Leisure PS leave or for broadcom AP.
...
//	printk("============>dtim is %x,RTLLIB_DTIM_UCAST is %x,RTLLIB_DTIM_MBCAST is %x,ieee->ps is %x\n",dtim ,RTLLIB_DTIM_UCAST,RTLLIB_DTIM_MBCAST,ieee->ps);
...
//#warning CHECK_LOCK_HERE
		...
		//printk("%s(3): pkt buffered in ap will awake ++++++++++ rtllib_sta_wakeup\n", __func__);
		rtllib_sta_wakeup(ieee,1);
...
#endif
{
	...
	case ACT_CAT_MESH_INTERWORKING:
		...
		default:
//			if (net_ratelimit())
//			RTLLIB_DEBUG(RTLLIB_DL_BA, "unknown action frame(%d)\n", tmp);
			break;
	}
	return;
}
		...
		case RTLLIB_STYPE_REASSOC_RESP:
...
#endif
			{
				...
				if (0 == (errcode=assoc_parse(ieee,skb, &aid))){
					...
					//get assocresp_ies for user space
					if(ieee->assocresp_ies){
...
//	ieee->stats.tx_packets++;
	...
	for(i = 0; i < txb->nr_frags; i++) {
		...
		if((queue_len  != 0) ||\
			...
			//rtllib_stop_queue(ieee);
#ifdef WIFI_TEST
			if (1) // queue every packet
#else
			if(queue_len < 200)
#endif
			{
				//printk("------------queue waitQ: q_idx=%d q_len=%ld skb->len=%d\n", queue_index, queue_len,txb->fragments[i]->len);
#ifdef USB_TX_DRIVER_AGGREGATION_ENABLE
...
void rtllib_start_ibss_wq(void *data)
{
	...
	if (ieee->state == RTLLIB_NOLINK){
...
#ifdef ENABLE_DOT11D 
		//if creating an ad-hoc, set its channel to 10 temporarily--
		//this is the requirement for ASUS, not 11D, so disable 11d.
		...
		//if(ieee->short_slot)
			//ieee->current_network.capability |= WLAN_CAPABILITY_SHORT_SLOT_TIME;
		//HTUseDefaultSetting(ieee); 
	}
...
#ifdef _RTL8192_EXT_PATCH_
	ieee->set_chan(ieee->dev, ieee->current_network.channel);  //YJ,recovered,090714
#endif
	ieee->link_change(ieee->dev);
...
void rtllib_disassociate(struct rtllib_device *ieee)
{
	...
	ieee->wap_set = 0;
	//ieee->ssid_set = 0;  //For GPIO auto-connect without UI when RF changed from OFF to ON.
	...
	//clark_mi added for android-x86
...
#else
void rtllib_softmac_stop_protocol(struct rtllib_device *ieee,u8 shutdown)//modified by amy for PS 090407
#endif
{
	ieee->sync_scan_hurryup = 1; 
	down(&ieee->wx_sem);
...
void rtllib_stop_protocol(struct rtllib_device *ieee, u8 shutdown)//modified by amy for PS 090407
{
	if (!ieee->proto_started)
		return;
	
	if(shutdown){//added by amy 090408 for IPS
	ieee->proto_started = 0;
		ieee->proto_stoppping = 1;
#ifdef ENABLE_IPS
		if(ieee->rtllib_ips_leave != NULL)
			ieee->rtllib_ips_leave(ieee->dev);//for RemoveAllTS will send pkt.
#endif
	}
	...
	//In these states, if UI or else cause set essid, 
	//We should change state to RTLLIB_NOLINK again, 
	//or something will fail (like roaming).
...
void rtllib_softmac_init(struct rtllib_device *ieee)
{
...
#endif
	ieee->bIsAggregateFrame = false;//added by amy for Leisure PS
	ieee->assoc_id = 0;
	ieee->queue_stop = 0;
	ieee->scanning = 0;
	...
	ieee->Regdot11TxHTOperationalRateSet[0]= 0xff;//support MCS 0~7
	ieee->Regdot11TxHTOperationalRateSet[1]= 0xff;//support MCS 8~15
	...
	sema_init(&ieee->ips_sem,1);//added by amy for IPS 090331	
...
//	sec.encrypt = 1;
 ...
 skip_host_crypt:
	...
	if (ret != 0)//error out
	{
		if(is_mesh)   //FIXME,YJ,090210, why free encryption info of all mps
		{
			for (i=0; i<MAX_MP; i++)
			{
