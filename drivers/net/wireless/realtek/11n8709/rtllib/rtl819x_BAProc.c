...
static struct sk_buff* rtllib_ADDBA(struct rtllib_device* ieee, u8* Dst, PBA_RECORD pBA, u16 StatusCode, u8 type)
{
	...
	if (pBA == NULL||ieee == NULL)
	{
        ...
        u32 Tmpaddr=0;
        int alignment=0;
        skb = dev_alloc_skb(len + sizeof( struct rtllib_hdr_3addr) + USB_512B_ALIGNMENT_SIZE); //need to add something others? FIXME
#else
	skb = dev_alloc_skb(len + sizeof( struct rtllib_hdr_3addr)); //need to add something others? FIXME
#endif
	if (skb == NULL)
	{
	...
	memset(skb->data, 0, sizeof( struct rtllib_hdr_3addr));  	//I wonder whether it's necessary. Apparently kernel will not do it when alloc a skb.
	...
	BAReq->frame_ctl = cpu_to_le16(RTLLIB_STYPE_MANAGE_ACT); //action frame

	//tag += sizeof( struct rtllib_hdr_3addr); //move to action field
	...
	)
{
	...
	//memset(skb->data, 0, len+sizeof( struct rtllib_hdr_3addr));  
	...
	Delba->frame_ctl = cpu_to_le16(RTLLIB_STYPE_MANAGE_ACT); //action frame
...
//   	     PBA_RECORD 	pBA	//BA_RECORD entry which stores the necessary information for BA 
//  output:  none
///notice: If any possible, please hide pBA in ieee. And temporarily use Manage Queue as 
///softmac_mgmt_xmit() usually does
//*****************************************************************************************/
void rtllib_send_ADDBAReq(struct rtllib_device* ieee, u8*	dst, PBA_RECORD	pBA)
{
	struct sk_buff *skb = NULL;
	skb = rtllib_ADDBA(ieee, dst, pBA, 0, ACT_ADDBAREQ); //construct ACT_ADDBAREQ frames so set statuscode zero.
...
//  output:  none
///notice: If any possible, please hide pBA in ieee. And temporarily use Manage Queue 
///as softmac_mgmt_xmit() usually does
//*********************************************************************************************/
void rtllib_send_ADDBARsp(struct rtllib_device* ieee, u8* dst, PBA_RECORD pBA, u16 StatusCode) 
{
	struct sk_buff *skb = NULL;
	skb = rtllib_ADDBA(ieee, dst, pBA, StatusCode, ACT_ADDBARSP); //construct ACT_ADDBARSP frames
	if (skb)
	{
		softmac_mgmt_xmit(skb, ieee);
		//same above
...
{
	struct sk_buff *skb = NULL;
	skb = rtllib_DELBA(ieee, dst, pBA, TxRxSelect, ReasonCode); //construct ACT_ADDBARSP frames
	if (skb)
	{
		softmac_mgmt_xmit(skb, ieee);
		//same above
...
///notice:  As this function need support of QOS, I comment some code out. And when qos is ready, 
///this code need to be support.
//*************************************************************************************************/
...
//   input:  struct sk_buff *   skb	//incoming ADDBAReq skb.
//  return:  0(pass), other(fail)
//notice:  As this function need support of QOS, I comment some code out. And when 
//qos is ready, this code need to be support.
//****************************************************************************************/
	...
	)
{
	...
	//FIXME: why block acks delay 3 packets? //YJ,090331, which will cause three packets lost when association is just established.
	//pBA->BaStartSeqCtrl.field.SeqNum = (pTS->TxCurSeq ) % 4096; 	
