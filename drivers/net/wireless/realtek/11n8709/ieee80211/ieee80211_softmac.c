...
void ieee80211_start_scan(struct ieee80211_device *ieee)
{
	...
	ieee->ieee80211_ips_leave_wq(ieee->dev);//added by amy for IPS 090331
#endif
...
void ieee80211_associate_procedure_wq(struct ieee80211_device *ieee)
{
	...
	if(ieee->ieee80211_ips_leave != NULL)
        	ieee->ieee80211_ips_leave(ieee->dev);//added by amy for IPS 090331
#endif
	down(&ieee->wx_sem);
...
//1) ieee80211_sta_ps_send_null_fram(epwr=1) called only the first time when enter Leisure PS and rcv beacon to sleep. 
//   or sta_sleep change from 0->2
//2) we don't call ieee80211_sta_ps_send_null_fram(epwr=1) every beacon sleep (sta_sleep = 1)
//3) after we have tell AP we sleep use ieee80211_sta_ps_send_null_frame(pwr=1), we will not call it(pwr=0) when wake up.
//4) we just call ieee80211_sta_ps_send_pspoll_frame to polling pkt buffered in AP to avoid AP send our pkt when we sleep.
//5) just call ieee80211_sta_ps_send_null_frame(pwr=0) when we Leisure PS leave or for broadcom AP.
...
//	printk("============>dtim is %x,IEEE80211_DTIM_UCAST is %x,IEEE80211_DTIM_MBCAST is %x,ieee->ps is %x\n",dtim ,IEEE80211_DTIM_UCAST,IEEE80211_DTIM_MBCAST,ieee->ps);		
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,5,0)					
				queue_delayed_work(ieee->wq, &ieee->associate_procedure_wq, 5); //2 is not ok
#else
				schedule_task(&ieee->associate_procedure_wq);
#endif
			}
...
void ieee80211_softmac_stop_protocol(struct ieee80211_device *ieee,bool shutdown)//modified by amy for PS 090407
{
	ieee->sync_scan_hurryup = 1; 
	down(&ieee->wx_sem);
...
void ieee80211_stop_protocol(struct ieee80211_device *ieee,bool shutdown)//modified by amy for PS 090407
{
	if (!ieee->proto_started)
		return;
...
#ifdef ENABLE_IPS
		if(ieee->ieee80211_ips_leave != NULL)
			ieee->ieee80211_ips_leave(ieee->dev);//for RemoveAllTS will send pkt.
#endif
	}
