	...
	)
{
	...
	switch(priv->rf_chip)
	{
		...
		switch( eRFPowerState )
		{
			...
			case eRfSleep:
			case eRfOff:					 
					//RXTX enable control : Deep Standby
	...
	)
{
	...
	{
		RT_TRACE(COMP_RF, "<--------- SetRFPowerState(): discard the request for eRFPowerState(%d) is the same.\n", eRFPowerState);
		return bResult;
	}

	bResult = SetRFPowerState8190(dev, eRFPowerState);
	
	RT_TRACE(COMP_RF, "<--------- SetRFPowerState(): bResult(%d)\n", bResult);
	...
	)
{
	...
	u16					RFWaitCounter = 0;
	unsigned long flag;
	RT_TRACE(COMP_RF, "===>MgntActSet_RF_State(): StateToSet(%d)\n",StateToSet);
	...
	while(true)
	{
		...
		if(priv->RFChangeInProgress)
		{
			...
			RT_TRACE(COMP_RF, "MgntActSet_RF_State(): RF Change in progress! Wait to set..StateToSet(%d).\n", StateToSet);
			
			// Set RF after the previous action is done. 
			while(priv->RFChangeInProgress)
			{
				RFWaitCounter ++;
				RT_TRACE(COMP_RF, "MgntActSet_RF_State(): Wait 1 ms (%d times)...\n", RFWaitCounter);
				udelay(1000); // 1 ms
	...
	case eRfOn:
		...
		if(! priv->RfOffReason)
		{
			...
			RT_TRACE(COMP_RF, "MgntActSet_RF_State - eRfon reject pMgntInfo->RfOffReason= 0x%x, ChangeSource=0x%X\n", priv->RfOffReason, ChangeSource);
	...
	if(bActionAllowed)
	{
		RT_TRACE(COMP_RF, "MgntActSet_RF_State(): Action is allowed.... StateToSet(%d), RfOffReason(%#X)\n", StateToSet, priv->RfOffReason);
	...
	{
		RT_TRACE(COMP_RF, "MgntActSet_RF_State(): Action is rejected.... StateToSet(%d), ChangeSource(%#X), RfOffReason(%#X)\n", StateToSet, ChangeSource, priv->RfOffReason);
	}
	...
	RT_TRACE(COMP_RF, "<===MgntActSet_RF_State()\n");
