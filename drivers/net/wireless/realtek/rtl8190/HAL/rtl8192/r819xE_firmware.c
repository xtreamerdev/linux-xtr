...
bool fw_download_code(struct net_device *dev, u8 *code_virtual_address, u32 buffer_len)
{
	...
	do {
		...
		skb_put(skb, i);
		//printk("priv->ieee80211->check_nic_enough_desc(dev,tcb_desc->queue_index) is %d, skb_queue_empty(&priv->ieee80211->skb_waitQ[tcb_desc->queue_index]) is %d, priv->ieee80211->queue_stop is %d\n",priv->ieee80211->check_nic_enough_desc(dev,tcb_desc->queue_index),skb_queue_empty(&priv->ieee80211->skb_waitQ[tcb_desc->queue_index]),priv->ieee80211->queue_stop);
