...
static void dm_check_rate_adaptive(struct net_device * dev)
{
	...
	if( !(priv->ieee80211->current_network.mode == WIRELESS_MODE_N_24G ||
		 priv->ieee80211->current_network.mode == WIRELESS_MODE_N_5G))
		 return;
...
{	//fill tx_cmd
	...
	for(i = 0;i <= 30; i++)
	{
		...
		else
		{
			...
			if(priv->CCKPresentAttentuation > -1&&priv->CCKPresentAttentuation <23)
			{
...
{	
	...
	if(priv->ieee80211->state == IEEE80211_LINKED)
	{
		if(priv->undecorated_smoothed_pwdb >= TX_POWER_NEAR_FIELD_THRESH_HIGH)
		{
		...
		else
		{
			...
			if(priv->undecorated_smoothed_pwdb < TX_POWER_NEAR_FIELD_THRESH_LOW && priv->bDynamicTxHighPower == true)
			{
				priv->bDynamicTxHighPower = false;
			}
