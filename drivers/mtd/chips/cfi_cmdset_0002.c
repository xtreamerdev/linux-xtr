...
static int __xipram chip_ready(struct map_info *map, unsigned long addr)
{
	...
	d = map_read_data(map, addr);
	t = map_read_data(map, addr);
...
static int __xipram chip_good(struct map_info *map, unsigned long addr, map_word expected)
{
	...
	oldd = map_read_data(map, addr);
	curd = map_read_data(map, addr);
	...
	case FL_ERASING:
		...
		for (;;) {
			...
			if (time_after(jiffies, timeo)) {
				...
				map_write_data(map, CMD(0x30), chip->in_progress_block_addr);
			...
			unsigned long adr)
{
	...
	(void) map_read_data(map, adr); /* ensure mmu mapping is up to date */
	local_irq_disable();
}
				...
				unsigned long adr)
{
	...
	if (chip->state != FL_POINT && chip->state != FL_READY) {
		map_write_data(map, CMD(0xf0), adr);
		chip->state = FL_READY;
	}
	(void) map_read_data(map, adr);
	xip_iprefetch();
	local_irq_enable();
}
...
static inline int do_read_onechip(struct map_info *map, struct flchip *chip, loff_t adr, size_t len, u_char *buf)
{
	...
	if (chip->state != FL_POINT && chip->state != FL_READY) {
		map_write_data(map, CMD(0xf0), cmd_addr);
		chip->state = FL_READY;
	}
...
static int __xipram do_write_oneword(struct map_info *map, struct flchip *chip, unsigned long adr, map_word datum)
{
 ...
 retry:
	...
	if (!chip_good(map, adr, datum)) {
		...
		map_write_data( map, CMD(0xF0), chip->start );
				  ...
				  size_t *retlen, const u_char *buf)
{
 ...
 retry:
		...
		tmp_buf = map_read_data(map, bus_ofs+chipstart);
 ...
 retry1:
		...
		tmp_buf = map_read_data(map, ofs + chipstart);
				    ...
				    int len)
{
	...
	/* Write Buffer Load */
	map_write_data(map, CMD(0x25), cmd_adr);

	chip->state = FL_WRITING_TO_BUFFER;

	/* Write length of data to come */
	words = len / map_bankwidth(map);
	map_write_data(map, CMD(words - 1), cmd_adr);
	/* Write data */
	z = 0;
	while(z < words * map_bankwidth(map)) {
		datum = map_word_load(map, buf);
		map_write_data(map, datum, adr + z);
	...
	map_write_data(map, CMD(0x29), cmd_adr);
	chip->state = FL_WRITING;
...
static int __xipram do_erase_oneblock(struct map_info *map, struct flchip *chip, unsigned long adr, int len, void *thunk)
{
	...
	cfi_send_gen_cmd(0xAA, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
	cfi_send_gen_cmd(0x55, cfi->addr_unlock2, chip->start, map, cfi, cfi->device_type, NULL);
	map_write_data(map, CMD(0x30), adr);
